...
public class ClassConcept {
	...
	public static void main(String[] args) {
		...
		System.out.println("클래스의 속성(Attribute) == 멤버 변수(멤버 필드)");
		...
		System.out.println("사람을 객체로 표현 했을 때, 자기 소개를 한다, 공부를 한다 같은 것들은 메소드 라고 할 수 있습니다.");
		
		System.out.println("메서드 선언 방법은 [접근제어자] [활용제어자] [반환타입] 메서드이름 ( [매개변수들] ) { ... } 과 같은 형식입니다. ");
		
		System.out.println("리턴타입은 메서드를 선언할 때 지정되고, 없으면 void 가 됩니다.");
		
		System.out.println("멤버메서드를 호출하기 위해서는 메서드가 선언되어 있는 클래스에 접근해야 합니다.");
		
		System.out.println("메서드를 호출하는 방법은 클래스/객체이름.메서드이름( [매개변수] ) 과 같은 형식으로 호출합니다.");
		
		System.out.println("생성자는 객체를 생성할때 실행되는 생성자 입니다. new 키워드와 함께 호출되며 클래스 이름과 동일합니다.");
		
		System.out.println("생성자는 반환타입을 갖지 않으며, 객체가 생성될 때 반드시 호출됩니다. 만약 하나도 정의되어 있지 않다면 default 기본생성자가 호출됩니다.");
